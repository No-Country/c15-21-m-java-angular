...
} from '../interfaces/shoping-cart.interface';
...
export class EcoTiendaService {
  ...
  //*********CARRITO *****************
  editProductShoppingCart(
    cartId: Number,
    productId: Number,
    quantity: Number
  ): Observable<any> {
    return this.http.put(
      `${this.url}/api/cart/public/carts/${cartId}/products/${productId}/quantity/${quantity}`,
      { cartId: cartId, productId: productId, quantity: Number }
    );
  }
  addToShoppingCart(
    cart: Number,
    ...
    return this.http.post(
      `${this.url}/api/cart/public/carts/${cart}/products/${product}/quantity/${quantity}?cartId=${cart}&productId=${product}&quantity=${quantity}`,
      { cart: Number, product: Number, quantity: Number }
    );
  }
  createShoppingCart(email: String): Observable<any> {
    return this.http.post(
      `${this.url}/api/cart/public/carts/user/${email}?email=${email}`,
      { email: email }
    );
  }

  getShoppingCartId(id: Number): Observable<ShoppingCartIdResponse[]> {
    return this.http.get<ShoppingCartIdResponse[]>(
      `${this.url}/api/cart/public/carts/${id}`
    );
  }
  getShoppingCartByEmail(email: String): Observable<any> {
    return this.http.get<any>(
      `${this.url}/api/cart/public/carts/email/${email}`
    );
  ...
  deleteProductShoppingCart(
    ...
    return this.http.delete<any>(
      `${this.url}/api/cart/public/carts/${cartId}/product/${productId}`
    );
  }
